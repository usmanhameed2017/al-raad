...
function Videos() 
{
  ...
  useSocket("VideoAdded", useCallback(addRealTime(setData), [setData]));
  useSocket("VideoUpdated", useCallback(updateRealTime(setData), [setData]));
  useSocket("VideoDeleted", useCallback(deleteRealTime(setData, setCurrentPage), [setData]));
  ...
  useEffect(() => {
    api.get({ url:`/video?page=${currentPage}&limit=${6}&search=${debouncedSearch}`, activateLoader:false })
